...
from dcsr.models import prune
from dcsr.models.kws_cnn import ds_cnn_model, train
from dcsr.models.preprocess_data import get_spectrogram
...
def train_kws_model(
    architecture: str, training_epochs: int, pruning_epochs: int, retraining_epochs: int, dense_weights_path: str
):
    ...
    model = ds_cnn_model(architecture, input_shape, num_labels)
    try:
        ...
        with mlflow.start_run(run_name="Train Base Model"):
            ...
            history = train(model, train_ds, val_ds, dense_weights_path, epochs=training_epochs)
            log_history(history)
...
def cli():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to weights for Dense Base Network."
            "Weights need to be generated by the same architecture as given in the 'architecture' parameter."
            "Providing weights skips the initial training of a dense base model."
        ),
    )
    parser.add_argument(
        "-d",
    ...
    train_kws_model(args.architecture, args.epochs, args.pruning_epochs, args.retraining_epochs, args.weights)
