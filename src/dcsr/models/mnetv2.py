...
def insert_layer_nonseq(model, layer_regex, insert_layer_factory, insert_layer_name=None, position="after"):
    ...
    for layer in model.layers[1:]:
        ...
        layer_input = [
            network_dict["new_output_tensor_of"][layer_aux] for layer_aux in network_dict["input_layers_of"][layer.name]
        ]
...
def train(model, train_ds, val_ds, epochs):
    ...
    early_stopping = callbacks.EarlyStopping(verbose=2, patience=5, monitor="val_accuracy", restore_best_weights=True)
