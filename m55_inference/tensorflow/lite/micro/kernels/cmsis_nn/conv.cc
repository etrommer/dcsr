    ...
    namespace
    {
            ...
            TfLiteEvalTensor *output, TfLiteEvalTensor *im2col, TfLiteSparsity *sparsity)
        {
            ...
            if (conv_params.dilation.h == 1 && conv_params.dilation.w == 1)
            {
                ...
                {
                    // Repack TFlite struct to something more lightweight
                    compressed_sparsity cp_sp = {
                        .bitmaps = sparsity->bitmaps,
                        .bitmasks = sparsity->bitmasks,
                        .delta_indices = sparsity->delta_indices,
                        .row_offsets = sparsity->row_offsets,
                        .group_minimums = sparsity->group_minimums,
                        .nnze = sparsity->nnze,
                    };
                    TFLITE_DCHECK_EQ(
                        sparse_convolve_wrapper_s8(
                            &ctx, &cp_sp, &conv_params, &quant_params, &input_dims,
                            tflite::micro::GetTensorData<int8_t>(input), &filter_dims,
                            tflite::micro::GetTensorData<int8_t>(filter), &bias_dims,
                            tflite::micro::GetTensorData<int32_t>(bias), &output_dims,
                            tflite::micro::GetTensorData<int8_t>(output)),
            ...
            // int cycle_count_after = ARM_PMU_Get_CCNTR();
            // RuntimeShape filter_shape = tflite::micro::GetTensorShape(filter);
            // if ((filter_shape.Dims(1) == 1) && (filter_shape.Dims(2) == 1))
            // {
                // TF_LITE_KERNEL_LOG(context, "Cycles PW Conv: %d", cycle_count_after - cycle_count_before);
            // }
#ifdef INTERMEDIATE_RESULTS
            // Calculate size of Conv node output tensor
            size_t output_dim = 1;
            for (size_t i = 0; i < output->dims->size; i++)
            {
                output_dim *= output->dims->data[i];
            }
            file_write_array(tflite::micro::GetTensorData<int8_t>(output), output_dim);
        ...
        TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node)
        {
            ...
            int filter_idx = node->inputs->data[kFilterTensor];
            const TfLiteTensor *filter_tensor = context->GetTensor(context, filter_idx);
            TfLiteSparsity *sparsity = filter_tensor->sparsity;
