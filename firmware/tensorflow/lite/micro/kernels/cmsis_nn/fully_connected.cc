    ...
    namespace
    {
                                       ...
                                       TfLiteEvalTensor *output)
        {
            ...
            if (nullptr != tflite::micro::GetTensorData<int32_t>(bias))
            {
                ...
                int filter_idx = node->inputs->data[kFullyConnectedWeightsTensor];
                ...
                else
                {
                    ...
                    TF_LITE_ENSURE_EQ(
                        context,
                        sparsely_connected_s8(
                            &ctx, &cp_sp, &fc_params, &quant_params, &input_dims,
                            tflite::micro::GetTensorData<int8_t>(input), &filter_dims,
                            tflite::micro::GetTensorData<int8_t>(filter), &bias_dims,
                            tflite::micro::GetTensorData<int32_t>(bias), &output_dims,
                            tflite::micro::GetTensorData<int8_t>(output)),
            ...
            // TF_LITE_KERNEL_LOG(context, "Cycles FC: %d", cycle_count_after - cycle_count_before);
