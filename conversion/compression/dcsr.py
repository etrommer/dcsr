...
import zlib
from . import estimate_references
...
def remove_slope(groups, slope):
    ...
    #     best_slope = 0
    #     for s in range(1,64):
    #         test = g - np.linspace(0, s * (len(g) -1), len(g))
    #         if (np.max(test) - np.min(test)) < best:
    #             best = np.max(test) - np.min(test)
    #             best_slope = s
    #     slopes.append(best_slope)

    # base = np.linspace(0, groups.shape[1]-1, groups.shape[1])[np.newaxis]
    # base = np.repeat(base, groups.shape[0], axis=0)
    # slopes = np.array(slopes)[np.newaxis]
    # base = base * slopes.T
...
def get_bit_position(groups, position=5):
    ...
    return (group_mask, mask[group_mask,:], np.where(mask, groups - 2**(position-1), groups))
...
def get_metrics(flat_rows, matrix):
    ...
    metrics['size_sparse_ri'], metrics['size_padding_ri'] = estimate_references.estimate_relative_indexing(matrix, 4)
...
def flatten_row_data(row_data):
    flat_dict = {}
    for key in ['values', 'delta_indices', 'minimums', 'bitmaps', 'bitmasks', 'bitwidths']:
    ...
    flat_dict['bitmaps'] = checked_conversion(pack_nibbles(
            flat_dict['bitmaps']
            .reshape(len(flat_dict['bitmaps']), 1)).flatten(), np.uint8)
...
def compress_row(row, slope=None, group_size=16, pad_to_groupsize=True):
    ...
    while(True):
        ...
        if len(values) == 0 or \
            ((overflow == False) and
            np.min(mins) > np.iinfo(np.int8).min and
            np.max(mins) < np.iinfo(np.int8).max and
            np.max(bitwidths) < 9):
            break
