...
The generated source can be used with the testing implementation to verify the correctness of results for a specific array
'''
...
def to_c_array(array, name, base_type, description=None):
    ...
    for line in range(num_lines):
        ans += "    " + ", ".join(['{:4d}'.format(a) for a in array[line*line_length:(line+1)*line_length]]) + ',\r\n'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate test file with C arrays from .npy array for testing'
    )
    ...
    compressed, _ = compress_matrix(matrix)
    ...
    args.output.write(to_c_array(matrix.flatten().astype(np.int8), 'reference', 'int8_t', 'Non-compressed array for comparision'))

    # dCSR data
    args.output.write(to_c_array(compressed['values'].astype(np.int8), 'values', 'int8_t', 'Compressed sparse values'))
    args.output.write(to_c_array(compressed['bitmaps'].astype(np.uint8), 'bitmaps', 'uint8_t', 'Extension Bitmaps'))
    args.output.write(to_c_array(compressed['bitmasks'].astype(np.uint16), 'bitmasks', 'uint16_t', 'Groupwise Bitmask'))
    args.output.write(to_c_array(compressed['delta_indices'].astype(np.uint8), 'delta_indices', 'uint8_t', 'Delta Index 4-Bit Base Values'))
    args.output.write(to_c_array(compressed['minimums'].astype(np.int8), 'minimums', 'int8_t', 'Groupwise Base pointer steps'))
    args.output.write(to_c_array(compressed['row_offsets'].astype(np.int16), 'row_offsets', 'int16_t', 'Deviation of Row length from average row length'))
    args.output.write(to_c_const(compressed['nnze'], 'nnze', 'uint32_t', 'Number of non-zero elements in the sparse matrix'))
    ...
    args.output.write(plain_array('group_buffer', 'int16_t', int(np.ceil(matrix.shape[1]/16)), 'Buffer for one row of base pointer steps'))
